package metropolis.xtracted.xtractedExplorer.repository
...
import metropolis.xtracted.xtractedExplorer.data.asSql
...
fun readIds(url: String, table: String, idColumn: DbColumnExplorer, filters: List<Filter<*>>, sortDirective: SortDirective) : List<Int> =
...
fun update(url: String, table: String, id: Int, setStatement: String, idColumn: DbColumnExplorer) : Unit =
    DriverManager.getConnection(url)
        .use {
            val logger: Logger = Logger.getLogger(Logger.GLOBAL_LOGGER_NAME)

            val sql = "UPDATE $table $setStatement WHERE ${idColumn.name} = $id"
...
fun count(url: String, table: String, idColumn: DbColumnExplorer) : Int =
    DriverManager.getConnection(url)
        .use {
            val logger: Logger = Logger.getLogger(Logger.GLOBAL_LOGGER_NAME)

            val sql = "SELECT COUNT(${idColumn.name}) FROM $table"
...
fun count(url : String, table: String, filters: List<Filter<*>>, idColumn: DbColumnExplorer) : Int =
    DriverManager.getConnection(url)
        .use {
            val logger: Logger = Logger.getLogger(Logger.GLOBAL_LOGGER_NAME)

            val sql = "SELECT COUNT(${idColumn.name}) FROM $table ${filters.asSql()}"
