...
import metropolis.xtracted.xtractedExplorer.data.OP

sealed class TableColumn<T, V>(val header       : String,
                               val width        : Dp,
                               val alignment    : Alignment,
                               val fixed        : Boolean,
                               val dbColumnExplorer     : DbColumnExplorer?,
...
class DoubleColumn<T>(header       : String,
                      ...
                      fixed        : Boolean        = false,
                      dbColumnExplorer     : DbColumnExplorer?      = null,
                      valueProvider: (T) -> Double?,
                      formatter    : (Double?) -> String = { it.toString() }) : TableColumn<T, Double>(header, width, alignment, fixed, dbColumnExplorer, valueProvider, formatter){
    ...
    override fun createFilter(): Filter<Double> {
        ...
        return Filter(dbColumnExplorer!!, op, args)
    }
}
...
class IntColumn<T>(header:        String,
                   ...
                   dbColumnExplorer:      DbColumnExplorer?    = null,
                   valueProvider: (T) -> Int?,
                   formatter :    (Int?) -> String = { it.toString() }) : TableColumn<T, Int>(header, width, alignment, fixed, dbColumnExplorer, valueProvider, formatter){
...
class StringColumn<T>(header:        String,
                      ...
                      formatter:     (String?) -> String = { it ?: "?" }) : TableColumn<T, String>(header, width, alignment, fixed, dbColumnExplorer, valueProvider, formatter) {

    override fun validFilterDescription(): Boolean = true

    override fun createFilter(): Filter<String> = Filter(dbColumnExplorer!!, OP.LIKE, listOf(if (filterAsText.contains("%")) filterAsText else "%$filterAsText%"))
}
