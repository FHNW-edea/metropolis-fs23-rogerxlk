package metropolis.shared.xtracted.controller.editor

import metropolis.shared.xtracted.controller.ControllerBase
import metropolis.shared.xtracted.controller.undo.UndoAction
import metropolis.shared.xtracted.controller.undo.UndoController
import metropolis.shared.xtracted.model.Translatable
import metropolis.shared.xtracted.model.editor.Attribute
import metropolis.shared.xtracted.model.editor.AttributeId
import metropolis.shared.xtracted.model.editor.EditorState
import metropolis.shared.xtracted.model.editor.ValidationResult
...
class EditorController<D : Identifiable>(
    val id: Int,                        // der EditorController verwaltet die Daten mit der ID 'id'
    ...
    ControllerBase<EditorState<D>, EditorAction>(
        initialState = EditorState(
            title = title,
            locale = locale,
            attributes = asAttributeList(repository.read(id)!!)
        ), // der Editor liest initial die Daten von der DB
        testMode = testMode
    ) {
    ...
    override fun executeAction(action: EditorAction): EditorState<D> =
        when (action) {
            ...
            is EditorAction.SetLocale -> setLocale(
                action.locale
            )
        }
    ...
    private fun update(attribute: Attribute<*>, valueAsText: String): EditorState<D> {
        val nextEditorState =
            state.copy(attributes = state.attributes.replaceAll(updateAffectedAttributes(attribute, valueAsText)))
                ...
                validationResult.valid -> {
                    ...
                    buildList {
                        ...
                        attribute.dependentAttributes.forEach { entry ->
                            ...
                            updatedDependentAttribute =
                                if (updatedDependentAttribute.required && updatedDependentAttribute.value == null) {
                                    updatedDependentAttribute.copy(
                                        validationResult = ValidationResult(
                                            false,
                                           ErrorMessage.REQUIRED
                                        )
                                    ...
                                    updatedDependentAttribute.copy(
                                        validationResult = updatedDependentAttribute.syntaxValidator(
                                            updatedDependentAttribute.valueAsText
                                        )
                                    )
                                }
